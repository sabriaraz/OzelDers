...
namespace OzelDers.CORE.Mappers
{
    public class MapperProfiles : Profile
    {
        public MapperProfiles()
        {
            ...
            // Egitmen Mapper
            CreateMap<Egitmen, EgitmenDTO>().ForMember(des => des.id, opt => opt.MapFrom(src => src.Id)).ForMember(des => des.ad, opt => opt.MapFrom(src => src.Ad)).ForMember(des => des.ilceId, opt => opt.MapFrom(src => src.IlceId)).ForMember(des => des.fiyat, opt => opt.MapFrom(src => src.Fiyat)).ForMember(des => des.ozgecmis, opt => opt.MapFrom(src => src.Ozgecmis)).ForMember(des => des.telefonNo, opt => opt.MapFrom(src => src.TelefonNo));
            CreateMap<EgitmenDTO, Egitmen>().ForMember(des => des.Id, opt => opt.MapFrom(src => src.id)).ForMember(des => des.Ad, opt => opt.MapFrom(src => src.ad)).ForMember(des => des.IlceId, opt => opt.MapFrom(src => src.ilceId)).ForMember(des => des.Fiyat, opt => opt.MapFrom(src => src.fiyat)).ForMember(des => des.Ozgecmis, opt => opt.MapFrom(src => src.ozgecmis)).ForMember(des => des.TelefonNo, opt => opt.MapFrom(src => src.telefonNo));
        }
    }
}
